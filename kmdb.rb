...
ari = Agent.create!(name: "Ari Gold")
christian_bale.update!(agent: ari)
...
# TODO!
Role.joins(:movie).includes(:movie, :actor).order("movies.year_released ASC", "roles.id ASC").each do |role|
...
# TODO!
Agent.find_by!(name: "Ari Gold").actors.each do |actor|
  puts actor.name
end